...
#####################################################(PLEASE DON'T MAKE INVALID MOVE)###########################################################################
												...
												###Here's the description of the code bellow###
The system construct from 3 Sequential Loops
First Loop  : 
X = 0 <<<>>>>{read from the camera and take a snapshoot of the board and save it with name (hello.jpg)}
X = 1 <<<>>>>{read the (hello.jpg) image, and apply corner detection, after this we will choose the board corners to apply perspective transformation}
X = 2 <<<>>>>{apply perspective transformation on the video image by choosing the board corners
			 ,after applying some filters we succeeded to detect the black pieces
			 ,now we have to images in the same Board state (1- image contains detected edges , 2- image contains detected black pieces)}
** genrate function that (genrate the state of the board)
	{function that take two parameters ((1- image contains detected edges , 2- image contains detected black pieces))}
	AND retrun the final state of the board**
after genration of the state of the board we genrate the applied move >> by taking every consecutive frames and subtract each other and genrate the Move 
'''
...
while x == 2 : 
	...
	#Save the image 
	cv2.imwrite('out.jpg',board_transform)
	...
	cv2.imwrite('mask.jpg',board_black)
